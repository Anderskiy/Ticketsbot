...
class TicketButton(disnake.ui.View):
    ...
    @disnake.ui.button(style=disnake.ButtonStyle.gray, label="What?", custom_id="what", emoji="❓")
    async def what(self, button: disnake.ui.Button, inter: disnake.ApplicationCommandInteraction):
        embed = disnake.Embed(title="What is it and why? 👀", colour=0x2B2D31)
        embed.add_field(name="", value="When a viewer wants to ask a question or report an issue, they can create a ticket, which will be automatically displayed in the channel below. This allows the server staff to respond to participants' questions and solve problems in a more organized way, which improves the quality of interaction with the audience. More briefly, I would call it a support center")
        embed.set_image( url='https://cdn.discordapp.com/attachments/1006260133162123338/1089553149943087185/space.png')
        await inter.response.send_message(embed=embed, ephemeral=True)
